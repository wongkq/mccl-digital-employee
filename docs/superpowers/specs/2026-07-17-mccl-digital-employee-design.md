...
- 不在NODE1/2/3上编译或改源码，这三台只接受scp的`libmccl.so`
...
attempt = 1..3:
  ...
  报告
    └→ 监督(report): 准确性
         REWORK → 打回报告（内循环≤2，不递增attempt），测试数据不重跑
         ABORT  → 停
...
编译循环与报告循环都是内循环，**不递增`attempt`**。只有监督(dev)和监督(test)判REWORK时才递增`attempt`——即`attempt`计的是"改代码→上集群验证"的完整轮次。
...
**ABORT条件汇总**：
- 越界行为：改`/opt/workspace/zb`之外的文件、重启远程、`git push`、在NODE1/2/3编译
- 编译内循环5轮不过
