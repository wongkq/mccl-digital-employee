...
Run:
```bash
# bait 在运行时拼装。若此处写点分四段的IP字面量，本计划文件自身就会被
# 不变式3抓到——检查扫描的是全部已跟踪文件，包括 docs/。
bait=$(printf '10.%s.%s.%s' 130 40 60)
printf '%s\n' "$bait" > ipbait.md && git add ipbait.md
