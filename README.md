...
| mpirun hang（判定见 `mccl-tester.md` §5） | 见`test-anomaly.md` | **禁止重启**（`references/mccl-safety.md`第3条，无例外）。agent会先采`dmesg`+IB状态落盘`test-anomaly.md`，再终止hang的本场景测试进程（kill审计同文件可见），然后按15分钟间隔重试同一命令至多5次（与驱动warm reset重试共用同场景额度，`agents/mccl-tester.md`第5节，重试日志`test-*.retry-<k>.log`）。额度耗尽则该场景判FAIL、转去跑另一场景--hang不再整轮停止（2026-08起）。你也别手动重启--禁重启这条不变 |
| 日志里出现 `MX_EVENTTYPE_DRIVER data: ResetType=1, ResetCause=1` / `mcCtxGetCurrent: Returned mcErrorDriverWarmReset` | 驱动warm reset（走自动重试的两类之一，另一类是hang） | agent会自行按15分钟间隔重试同一命令至多5次（`agents/mccl-tester.md`第5节重试规程），每次尝试（含重试）的记录都在`test-result.md`，重试日志为`test-*.retry-<k>.log`。5次均失败则该场景判FAIL并转去跑另一个场景--单场景失败（含其他故障，如SegFault）不中断整轮，两个场景各自独立判定。hang同样走自动重试且与warm reset共用同场景额度（见上一行） |
