...
/plugin marketplace add wongkq/mccl-digital-employee
...
第一条的`wongkq/mccl-digital-employee`是`owner/repo`简写；若你的CLI版本不认简写，改用完整地址`/plugin marketplace add https://github.com/wongkq/mccl-digital-employee.git`。第二条的格式是`插件名@marketplace名`，本仓库两者同名，所以是`mccl-digital-employee@mccl-digital-employee`。
